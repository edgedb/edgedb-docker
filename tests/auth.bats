...
@test "default user plain password edgedb" {
  local container_id
  local instance
...
@test "default user plain password" {
  ...
  create_instance container_id instance '{"password":"test2"}' \
    --env=GEL_SERVER_PASSWORD=test2

  run edgedb -I "${instance}" query "SELECT 7+7"
  [[ ${lines[-1]} = "14" ]]
}

@test "new user plain password edgedb" {
  local container_id
  local instance
...
@test "default user hashed password edgedb" {
  local container_id
  local instance
...
@test "default user hashed password" {
  ...
  create_instance container_id instance '{"password":"test3"}' \
    --env='GEL_SERVER_PASSWORD_HASH=SCRAM-SHA-256$4096:rEQ2xuv6ASCA61VMaqU9yg==$uvda3+u+zewd/GvbIofDjk5EEReNJ0KRhLX0001bVRQ=:sdF5jXfPMnM9GNu+JC39fV4Pa5oZEULEm8cdDRZMJDw='
...
@test "create role manually (via env) edgedb" {
  local container_id
  local instance
...
@test "create role manually (via env)" {
  ...
  create_instance container_id instance '{"user":"test4", "password":"test4"}' \
    --env=GEL_SERVER_BOOTSTRAP_COMMAND="CREATE SUPERUSER ROLE test4 { SET password := 'test4'; };"
...
@test "custom default branch edgedb" {
  local container_id
  local instance
...
@test "tls in env vars" {
  ...
  create_instance container_id instance '{"tls_ca_file":"tests/compose/certs/ca.pem"}' \
    --env=GEL_SERVER_TLS_KEY="$(cat tests/compose/certs/server_key.pem)" \
    --env=GEL_SERVER_TLS_CERT="$(cat tests/compose/certs/server_cert.pem)"
